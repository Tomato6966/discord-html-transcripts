...
async function renderASTNode(node, context) {
    ...
    switch (type) {
        ...
        case 'channel': {
            ...
            return (react_1.default.createElement(discord_components_react_1.DiscordMention, { type: channel ? (channel instanceof discord_js_1.DMChannel ? 'channel' : getChannelType(channel.type)) : 'channel' }, channel ? (channel instanceof discord_js_1.DMChannel ? 'DM Channel' : channel.name) : `<#${id}>`));
        }
        case 'role': {
            const id = node.id;
